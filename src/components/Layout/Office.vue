...
const { setCurrent, links, load, current } = store;
...
const menu = [
  ...
  { label: 'Филиалы', name: 'Departments', icon: OfficeBuildingIcon },
...
const departments = computed(() => {
  return links.value.map(({ label, id }) => ({ label: label, value: id }))
})

const department = ref(current)
          ...
          <div class="flex items-center">
            <Dropdown :items="userMenu" direction="right" class="ml-3 relative">
              <MenuButton class="max-w-xs bg-white flex items-center text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                <img class="h-8 w-8 rounded-full object-cover inline-block" :src="user.avatar"/>
              </MenuButton>
            </Dropdown>
          </div>
        </div>
