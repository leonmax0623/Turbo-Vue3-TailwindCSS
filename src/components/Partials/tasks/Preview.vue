...
<div class="grid grid-cols-1 gap-6 lg:grid-flow-col-dense lg:grid-cols-6">
  <div class="space-y-6 lg:col-start-1 lg:col-span-4">
    <Card>
        <template #header>
            <div class="flex gap-3 flex-wrap justify-between items-center">
                <Avatar
                  v-if="task.user"
                  :title="`${task.user?.name ?? ''} ${task.user?.middle_name ?? ''}`"
                  :subtitle="task.user?.office_position"
    ...
    <Comments title="Комментарии к задаче / Заметки" model="task" :id="route.params.id" class="mt-2" :disabled="!canTasks(task, 'update')" />
  </div>
  <div class="lg:col-start-5 lg:col-span-2 relative">
    <Timeline
      :service="service"
    />
  </div>
