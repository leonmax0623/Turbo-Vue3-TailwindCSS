...
const navigationGuards = (to, from, next) => {
  ...
  const nextRoute = userRole.value?.name === 'slecar' ? { name: 'WorderKanban' } : to.query.target ?? '/orders'; /* 'dashboard' */
...
export const PERMISSIONS = {
  TASKS: {
    READ: () => has('read tasks'),
    READ_DEPARTMENT: () => (has('read department tasks') || has('read department own tasks')) && !has('read tasks'),
